    ...
    for idx, time in enumerate(data_df['time'].unique()):
        ...
        my_cmap = cmx.ScalarMappable(
            norm=mcolors.Normalize(
                vmin=0,
                vmax=data_df.loc[data_df['time'] == time, 'green/blue_norm'].max()
            ),
...
def normalize_by_sum_at_180min(data_row, signal_at_180):

    signal_of_repeat = signal_at_180[signal_at_180['repeat'] == data_row['repeat']]['green/blue'].iloc[0]

    data_row['green/blue_norm'] = data_row['green/blue'] / signal_of_repeat
    return data_row
...
def main(experiments, test):
    ...
    data_df = data_df.loc[(data_df['repeat'] != 'n2')] # no 400 ng and no 240 min
    data_df = data_df.loc[(data_df['repeat'] != 'n1')] # no 400 ng
    data_df = data_df.loc[(data_df['repeat'] != 'n5')] # didn't work

    # normalize the green via blue
    data_df['green/blue'] = data_df['green_pixel']/data_df['blue_pixel']

    # normalize by dividing through the sum green/blue of the experiment at 180 min

    # Take only values that are in every repeat:
    data_df = data_df.loc[data_df['condition'].isin([200, 100, 50, 10, 0])]

    test_df = data_df[['condition', 'time', 'repeat']]

    signal_180min = data_df[['repeat', 'green/blue']].groupby('repeat', as_index=False).sum()

    data_df = data_df.apply(normalize_by_sum_at_180min, axis = 1, args= (signal_180min, )) # https://stackoverflow.com/questions/37630202/python-pandas-passing-in-dataframe-to-df-apply
    ...
    mean_signal_df = data_df[['green/blue_norm', 'condition', 'time', 'cond_idx', 'time_idx']]

    mean_signal_df = mean_signal_df.groupby(
        by=['condition', 'time', 'cond_idx', 'time_idx'],
        as_index=False
    ).agg(
        read_out=('green/blue_norm', lambda x: list(x))
    )
    ...
    for idx, time in enumerate([240, 180, 120]):
        ...
        two_hundred = data_min_df.loc[data_min_df['condition'] == 200, 'green/blue_norm'].values
        one_hundred = data_min_df.loc[data_min_df['condition'] == 100, 'green/blue_norm'].values
        zero = data_min_df.loc[data_min_df['condition'] == 0, 'green/blue_norm'].values

        plt.sca(ax)
        height = data_min_df['green/blue_norm'].max()
        ...
        plt.hlines(xmin=0.1, xmax=3.9, y=height * 1.25, linewidth=2, color='black')
        ...
        plt.hlines(xmin=0.1, xmax=0.9, y=height * 1.1, linewidth=2, color='black')
        ...
        plt.ylim(0, height * 1.4)
        ax.yaxis.set_major_formatter(FormatStrFormatter('%.2f'))  # two decimal points
